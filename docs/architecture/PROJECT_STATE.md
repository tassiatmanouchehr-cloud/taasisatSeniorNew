...
| Current Test Count | **1100 passing**, 0 failing (`python manage.py test`, run on this branch — 76 new tests over the `main` baseline of 1024: 22 for the new `apps.provider_portal` app, 16 for the new `apps.organization_portal` app, 14 for `ProviderAssignmentActionService` (`apps.booking.tests.test_provider_actions`), 8 for `ProviderExecutionService` (`apps.execution.tests.test_provider_actions`), and 16 spread across extensions to existing apps: `apps.portal` (dashboard upcoming-visits, request-list filters, care-recipient archive), `apps.accounts` (`CareRecipientService.archive()`, event publishing), and the two new `*PortalOrmDisciplineTest` guardrails in `apps.kernel.tests.test_architecture_guardrails`) |
