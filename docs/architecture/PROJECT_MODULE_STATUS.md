...
| 08 | Identity, Roles, Profiles & Access Engine | Identity, auth, profiles, role assignment, permissions, org affiliation | **Mostly Complete** | `kernel`, `accounts` | #3 (RBAC), #19 (auth UX/multi-role) + Epic 02 (org staff lifecycle) | `8cf5442`, `870ddad` | kernel 17 + accounts 9 = 26 files | The most mature demand-side subsystem. Email is now the Django-admin login identifier (`USERNAME_FIELD`), UUID no longer exposed as one. Multi-role identity via idempotent `ensure_*_profile()` helpers, no duplicate accounts. Epic 02 added `OrganizationStaffService` (`apps.accounts.services.organization_staff`) — approve/suspend using `OrganizationMembership`'s pre-existing `status`/`approved_by`/`joined_at` fields, no new membership model — and `provider_identity`/`organization_identity` resolvers used as the sole identity bridge for `apps.provider_portal`/`apps.organization_portal`. | No permission-key registry (freeform strings); default seeded roles ship with empty `permissions` generally (fixed for the dev `platform-owner` specifically, not fixed platform-wide); OTP delivery is console-only. The two new portals deliberately use ownership, not RBAC keys, as their security boundary — see `DECISION_HISTORY.md`. | — (foundational) | RBAC Hardening & Permission Registry |
