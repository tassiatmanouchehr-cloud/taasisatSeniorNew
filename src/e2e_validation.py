...
from apps.kernel.models import Tenant, Person, UserAccount, Role, RoleAssignment, ServiceSupplier
from apps.accounts.models import CustomerProfile, ElderProfile, CaregiverProfile
...
from apps.orders.models import ServiceCategory, Order
...
def create_customer():
    ...
    profile = CustomerProfile.objects.create(
        user=user, person=person, phone=f'+98912999{suffix[:4]}',
        display_name='E2E Customer', city='Tehran', status='ACTIVE'
    )
    # Grant the customer the necessary permissions for the full workflow
    role, _ = Role.objects.get_or_create(
        tenant=tenant, slug='e2e-customer',
        defaults={
            'name': 'E2E Customer',
...
def create_caregiver():
    ...
    profile = CaregiverProfile.objects.create(
        user=user, person=person, phone=f'+98912998{suffix[:4]}',
        display_name='E2E Caregiver', city='Tehran',
        specialty='Elder Care', status='ACTIVE'
    )
    return user, profile
...
def create_order():
    cat, _ = ServiceCategory.objects.get_or_create(
        tenant=tenant, slug='elder-care',
        defaults={'name': 'Elder Care'}
    )
...
def provider_accept():
    return ProviderAssignmentActionService.confirm(
