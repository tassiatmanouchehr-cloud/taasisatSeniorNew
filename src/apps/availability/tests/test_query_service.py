...
class AvailabilityQueryServiceTest(AvailabilityTestCase):
    ...
    def test_available_within_working_window(self):
        self.assertTrue(
            AvailabilityQueryService.is_supplier_available(
                supplier=self.supplier,
                start=self._aware(self.monday, 10),
                end=self._aware(self.monday, 11),
            ),
        )
    ...
    def test_unavailable_outside_working_window(self):
        self.assertFalse(
            AvailabilityQueryService.is_supplier_available(
                supplier=self.supplier,
                start=self._aware(self.monday, 18),
                end=self._aware(self.monday, 19),
            ),
        )
    ...
    def test_unavailable_when_request_partially_exceeds_window(self):
        self.assertFalse(
            AvailabilityQueryService.is_supplier_available(
                supplier=self.supplier,
                start=self._aware(self.monday, 16),
                end=self._aware(self.monday, 18),
            ),
        )
    ...
    def test_unavailable_on_day_with_no_configured_window(self):
        ...
        self.assertFalse(
            AvailabilityQueryService.is_supplier_available(
                supplier=self.supplier,
                start=self._aware(tuesday, 10),
                end=self._aware(tuesday, 11),
            ),
        )
    ...
    def test_blocked_period_overrides_working_window(self):
        ...
        self.assertFalse(
            AvailabilityQueryService.is_supplier_available(
                supplier=self.supplier,
                start=self._aware(self.monday, 10, 30),
                end=self._aware(self.monday, 11),
            ),
        )
        ...
        self.assertTrue(
            AvailabilityQueryService.is_supplier_available(
                supplier=self.supplier,
                start=self._aware(self.monday, 13),
                end=self._aware(self.monday, 14),
            ),
        )
    ...
    def test_overlapping_blocked_periods_both_apply(self):
        ...
        AvailabilityMutationService.add_blocked_period(
            supplier=self.supplier,
            start_at=self._aware(self.monday, 11),
            end_at=self._aware(self.monday, 13),
        )

        self.assertEqual(
            AvailabilityQueryService.get_blocked_periods(
                supplier=self.supplier,
                start=self._aware(self.monday, 0),
                end=self._aware(self.monday, 23),
            ).count(),
            2,
        )
        self.assertFalse(
            AvailabilityQueryService.is_supplier_available(
                supplier=self.supplier,
                start=self._aware(self.monday, 11, 30),
                end=self._aware(self.monday, 12, 30),
            ),
        )
    ...
    def test_start_after_end_raises_availability_error(self):
        with self.assertRaises(AvailabilityError):
            AvailabilityQueryService.is_supplier_available(
                supplier=self.supplier,
                start=self._aware(self.monday, 11),
                end=self._aware(self.monday, 10),
            )
    ...
    def test_range_spanning_local_midnight_raises_availability_error(self):
        ...
        with self.assertRaises(AvailabilityError):
            AvailabilityQueryService.is_supplier_available(
                supplier=self.supplier,
                start=self._aware(self.monday, 23),
                end=self._aware(tuesday, 1),
            )
    ...
    def test_utc_datetime_is_converted_to_local_time_for_window_matching(self):
        ...
        utc_start = local_aware.astimezone(dt.UTC)
        ...
        self.assertTrue(
            AvailabilityQueryService.is_supplier_available(
                supplier=self.supplier,
                start=utc_start + dt.timedelta(hours=3),
                end=utc_end + dt.timedelta(hours=3),
            ),
        )
