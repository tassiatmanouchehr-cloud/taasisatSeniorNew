...
class QuoteServiceBasePriceTest(PricingTestCase):
    def test_explicit_base_amount_bypasses_rule_lookup(self):
        quote = QuoteService.generate_quote(
            tenant_id=self.tenant.id, service_category=self.category, base_amount=Decimal("123456"),
        )
    ...
    def test_fixed_amount_rule_used_as_base(self):
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Base", rule_type=PricingRuleType.FIXED_AMOUNT,
            amount=Decimal("500000"), service_category=self.category,
        )
    ...
    def test_hourly_rate_rule_multiplies_by_duration(self):
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Hourly", rule_type=PricingRuleType.HOURLY_RATE,
            amount=Decimal("100000"), service_category=self.category,
        )
        quote = QuoteService.generate_quote(
            tenant_id=self.tenant.id, service_category=self.category, duration_hours=Decimal("3"),
        )
    ...
    def test_supplier_override_takes_precedence_over_category_default(self):
        ...
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Category default", rule_type=PricingRuleType.FIXED_AMOUNT,
            amount=Decimal("400000"), service_category=self.category,
        )
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Supplier override", rule_type=PricingRuleType.FIXED_AMOUNT,
            amount=Decimal("650000"), service_category=self.category, supplier=supplier,
        )

        quote = QuoteService.generate_quote(
            tenant_id=self.tenant.id, service_category=self.category, supplier=supplier,
        )
    ...
    def test_organization_supplier_override_works_identically(self):
        ...
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Org override", rule_type=PricingRuleType.FIXED_AMOUNT,
            amount=Decimal("900000"), service_category=self.category, supplier=org_supplier,
        )

        quote = QuoteService.generate_quote(
            tenant_id=self.tenant.id, service_category=self.category, supplier=org_supplier,
        )
    ...
    def test_tenant_wide_default_used_when_no_scoped_rule_matches(self):
        supplier = self._create_supplier()
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Tenant default", rule_type=PricingRuleType.FIXED_AMOUNT,
...
class QuoteServiceModifierTest(PricingTestCase):
    ...
    def test_flat_surcharge_applied(self):
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Travel fee", rule_type=PricingRuleType.FLAT_SURCHARGE,
    ...
    def test_percentage_adjustment_applies_to_running_total(self):
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="10% bump", rule_type=PricingRuleType.PERCENTAGE_ADJUSTMENT,
    ...
    def test_surcharge_ordering_changes_the_result(self):
        ...
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Flat", rule_type=PricingRuleType.FLAT_SURCHARGE,
            amount=Decimal("10000"), priority=0, service_category=self.category,
        )
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Pct", rule_type=PricingRuleType.PERCENTAGE_ADJUSTMENT,
            percentage=Decimal("10"), priority=1, service_category=self.category,
        )
    ...
    def test_overlapping_time_of_day_surcharges_both_apply(self):
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Morning surcharge", rule_type=PricingRuleType.TIME_OF_DAY_SURCHARGE,
            amount=Decimal("5000"), time_start=dt.time(8, 0), time_end=dt.time(12, 0), priority=0,
        )
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Late-morning surcharge", rule_type=PricingRuleType.TIME_OF_DAY_SURCHARGE,
            amount=Decimal("3000"), time_start=dt.time(10, 0), time_end=dt.time(14, 0), priority=1,
        )
        quote = QuoteService.generate_quote(
            tenant_id=self.tenant.id, service_category=self.category, base_amount=Decimal("100000"),
    ...
    def test_time_of_day_surcharge_does_not_apply_outside_window(self):
        PricingRuleService.create_rule(
            ...
            amount=Decimal("5000"), time_start=dt.time(22, 0), time_end=dt.time(23, 59),
        )
        quote = QuoteService.generate_quote(
            tenant_id=self.tenant.id, service_category=self.category, base_amount=Decimal("100000"),
            requested_at=self._aware(self.monday, 10),
    ...
    def test_weekend_surcharge_applies_on_default_weekend_day(self):
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Weekend surcharge", rule_type=PricingRuleType.WEEKEND_SURCHARGE,
    ...
    def test_holiday_surcharge_requires_explicit_flag(self):
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Holiday surcharge", rule_type=PricingRuleType.HOLIDAY_SURCHARGE,
            amount=Decimal("25000"),
        )
        quote = QuoteService.generate_quote(
            tenant_id=self.tenant.id, service_category=self.category, base_amount=Decimal("100000"),
            requested_at=self._aware(self.monday, 10), is_holiday=False,
        )
        self.assertEqual(quote.total_amount, Decimal("100000.00"))
    ...
    def test_holiday_surcharge_applies_when_flagged(self):
        ...
        quote = QuoteService.generate_quote(
            tenant_id=self.tenant.id, service_category=self.category, base_amount=Decimal("100000"),
            requested_at=self._aware(self.monday, 10), is_holiday=True,
        )
...
class QuoteServiceDeterminismAndRoundingTest(PricingTestCase):
    def test_generate_quote_is_deterministic(self):
        ...
        quote_a = QuoteService.generate_quote(
            tenant_id=self.tenant.id, service_category=self.category, base_amount=Decimal("99999.99"),
    ...
    def test_percentage_rounds_half_up(self):
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Odd percentage", rule_type=PricingRuleType.PERCENTAGE_ADJUSTMENT,
            percentage=Decimal("1"),
        )
        # 100.005 * 1.01 = 101.00505 -> rounds to 101.01 (ROUND_HALF_UP on the final quantize)
        quote = QuoteService.generate_quote(
            tenant_id=self.tenant.id, service_category=self.category, base_amount=Decimal("100.005"),
        )
    ...
    def test_quote_breakdown_lines_match_pricing_snapshot(self):
        PricingRuleService.create_rule(
            tenant_id=self.tenant.id, name="Flat", rule_type=PricingRuleType.FLAT_SURCHARGE, amount=Decimal("5000"),
        )
        quote = QuoteService.generate_quote(
            tenant_id=self.tenant.id, service_category=self.category, base_amount=Decimal("100000"),
        )
    ...
    def test_quote_order_reference(self):
        from apps.orders.models import CatalogStatus, Order, OrderSource, OrderStatus
        ...
        quote = QuoteService.generate_quote(
            tenant_id=self.tenant.id, service_category=self.category, base_amount=Decimal("100000"), order=order,
        )
