"""Shared fixtures for finance engine tests (not a test module itself)."""
...
class FinanceTestCase(TestCase):
    """Base test case: a tenant, a customer-billed order, an assigned supplier, and helpers to close execution."""

    def setUp(self):
        self.tenant = Tenant.objects.create(slug=f"finance-{uuid.uuid4().hex[:8]}", name="Finance Test Tenant")
    ...
    def _close_execution_session(self, *, supplier_assignment=None):
