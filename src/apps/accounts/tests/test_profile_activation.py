"""ProfileActivationService — Phase 1.3 Parts B, C, E."""
...
class EligibleCaregiverActivationTest(_ActivationFixtureMixin, TestCase):
    ...
    def test_eligible_caregiver_is_activated(self):
        ...
        self.assertTrue(
            ProfileActivationService.is_activated(
                resource_type="CaregiverProfile", resource_id=self.caregiver.id, tenant_id=self.tenant.id,
            ),
        )

    def test_activation_creates_audit_record(self):
        self._approve_required_caregiver_documents()
...
class EligibleOrganizationActivationTest(_ActivationFixtureMixin, TestCase):
    ...
    def test_eligible_organization_is_activated(self):
    ...
    def test_repeated_activation_is_idempotent(self):
        self._approve_required_organization_documents()
        ProfileActivationService.activate_organization(self.organization.id, tenant_id=self.tenant.id, actor=self.reviewer)
        ProfileActivationService.activate_organization(self.organization.id, tenant_id=self.tenant.id, actor=self.reviewer)
        count = AuditLog.objects.filter(
...
class IneligibleActivationRefusalTest(_ActivationFixtureMixin, TestCase):
    ...
    def test_ineligible_caregiver_activation_raises_controlled_error_with_reasons(self):
...
class ConcurrentActivationTest(_ActivationFixtureMixin, TransactionTestCase):
    ...
    profile must leave exactly one effective activation, not two."""
