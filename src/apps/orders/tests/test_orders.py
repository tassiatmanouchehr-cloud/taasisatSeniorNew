...
class BaseOrderTest(TestCase):
    def setUp(self):
        self.tenant, _ = Tenant.objects.get_or_create(
            slug="salmandyar", defaults={"name": "Test"}
        )
        self.person = Person.objects.create(
            tenant=self.tenant, full_name="Test Person"
        )
        ...
        self.cg_person = Person.objects.create(
            tenant=self.tenant, full_name="Caregiver"
        )
        self.cg_user = UserAccount.objects.create_user(
            phone="09130000001", person=self.cg_person, tenant=self.tenant
        )
...
class CatalogTest(BaseOrderTest):
    ...
    def test_inactive_category_rejected(self):
        with self.assertRaises(OrderValidationError):
            create_public_order(**self._make_order_kwargs(
                service_category_id=self.inactive_cat.id
            ))
    ...
    def test_inactive_type_rejected(self):
        with self.assertRaises(OrderValidationError):
            create_public_order(**self._make_order_kwargs(
                service_type_id=self.inactive_type.id
            ))
...
class AssignmentTest(BaseOrderTest):
    ...
    def test_replace_provider_waiting(self):
        ...
        cg2_user = UserAccount.objects.create_user(
            phone="09130000002", person=cg2_person, tenant=self.tenant
        )
