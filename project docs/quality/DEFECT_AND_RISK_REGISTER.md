...
| KL-008 | ~~No activation wiring / completion auto-recompute~~ — **RESOLVED (Phase 1.3)** | `ProfileActivationService.activate_caregiver()/activate_organization()` now calls `ActivationEligibilityService.evaluate()` and records a controlled, audited, permission-gated activation (BG-018, ARCHITECTURE_DECISION_LOG ADM-016). `ProfileCompletionService` is now the single deterministic source for completion percentage. Superseded item: profile `verification_status` roll-up itself is IMPLEMENTED (Phase 1.2), not a limitation. |
