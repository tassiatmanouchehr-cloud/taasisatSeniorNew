...
`fix/public-caregiver-directory-visibility`; see FR-015 below. **Phase 5 has not started.**
**Last verified date:** 2026-07-14 (full sweep); 2026-07-16 (targeted re-evaluation); 2026-07-17 (Phase 4 Closure Review); 2026-07-18 (Core Profile-ServiceSupplier Invariant Remediation + independent pre-merge review remediation + PR #18 merge + FR-015 caregiver-directory tenant-hint fix)
...
### FR-015: Public Caregiver Directory Ignores `?tenant=` Hint, Returning Zero Results — **RESOLVED 2026-07-18**

**Severity:** HIGH
**Confidence:** HIGH (reproduced against a real running server, root-caused via direct code inspection, fixed with a failing-then-passing regression test)
**Affected:** `apps/public_site/views.py` (`find_a_caregiver`)
**Evidence:** `apps.public_site.views.caregiver_profile()` (the public caregiver *detail* view) resolves an explicit `?tenant=<slug>` query hint via `_resolve_optional_tenant_hint()` and passes the resolved `tenant_id` into `CaregiverPublicProfileService.get_profile()` — added specifically so a supplier living in a non-default tenant (e.g. `seed_product_walkthrough`'s own dedicated demo tenant) could be previewed. `find_a_caregiver()` (the *directory/listing* view) never adopted the same hint resolution: it called `CaregiverDirectoryService.search()` without a `tenant_id` argument at all, so the service silently defaulted to `TenantService.get_default_tenant_id()` regardless of the query string. A caregiver in any non-default tenant (proven ACTIVE, VERIFIED, ACTIVE `ServiceSupplier`, correct type — publicly visible per `common.is_publicly_visible_attrs()`, confirmed 200 + fully rendered at the direct detail URL) was therefore invisible in the directory for that same tenant: `GET /find-a-caregiver/?tenant=demo-senior-platform` rendered `0 مراقب یافت شد` with no filters applied, while `GET /find-a-caregiver/<id>/?tenant=demo-senior-platform` rendered the same caregiver correctly. Root-cause proven stage-by-stage: querying `SupplierSearchService.filter_suppliers()` with `tenant_id=TenantService.get_default_tenant_id()` excluded the caregiver at the very first stage; querying with `tenant_id=<demo tenant>` included it through every subsequent stage. `CaregiverDirectoryService.search()` itself already accepted and correctly used a `tenant_id` keyword argument (confirmed by its own existing service-level tests, which pass `tenant_id` explicitly) — the defect was entirely in the view layer failing to pass it through, not in the service or query layer. `find_an_organization()` has the identical gap (never resolves the hint either) but is unrelated to this fix and was not touched — the organization directory's current emptiness for the same demo tenant is separately caused by its seeded organizations having `verification_status=unverified` (see Remaining Known Risks under ADM-029's post-merge closure).
**Runtime impact:** A caregiver correctly activated, verified, and supplier-synchronized by the Core Profile-ServiceSupplier Invariant Remediation (FR-014/PR #18) was still invisible in the one surface most visitors actually use — the directory listing — even though its own direct profile URL worked. Confirmed via a real local server against `seed_product_walkthrough`'s demo tenant before any fix: directory 0 results, detail page 200 with full render.
**Resolution:** `find_a_caregiver()` now calls `_resolve_optional_tenant_hint(request)` (the same helper `caregiver_profile()` already used) and passes the resolved `tenant_id` into `CaregiverDirectoryService.search()` — a two-line change, no service/query-layer change, no visibility-rule change. Callers that never pass `?tenant=` get exactly the pre-existing default-tenant behavior; an unknown slug now correctly 404s instead of silently rendering an empty list, matching the detail view's own contract. 8 new regression tests (`FindACaregiverViewTenantHintTest`, mirroring the existing `CaregiverProfileTenantHintTest`): correct hint lists the caregiver; missing hint still excludes a non-default-tenant caregiver; unknown slug 404s; wrong tenant hint cannot cross-tenant-leak; unverified/DRAFT/inactive-supplier caregivers remain excluded even with a correct hint; and a consistency test proving a caregiver visible via the detail-page hint is also visible via the directory hint for the same unfiltered request. Full regression 2321 -> 2329/2329 green, 0 failures, 0 errors, no warnings. Branch `fix/public-caregiver-directory-visibility`, PR opened (not merged).
