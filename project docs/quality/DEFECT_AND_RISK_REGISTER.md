...
| KL-010 | Caregiver gallery, financial overview, orders + history not implemented | Roadmap Phase 2's full scope; Phase 2.1 (2026-07-15) delivered only the foundation slice (skills, experience, verified-credential summary, corrected public-profile eligibility). See `quality/COMPLETION_BACKLOG.md` BG-021. |
| KL-011 | Directory/home-page caregiver listings do not require verification/account-active | Phase 2.1 (2026-07-15) added `verification_status == VERIFIED` and account `is_active` checks to the single caregiver public profile page only, not to the shared `common.is_publicly_visible()` function the caregiver directory/home-page listings also use — a caregiver can appear in a listing while their own profile page 404s. Deliberate, scoped decision (`ARCHITECTURE_DECISION_LOG.md` ADM-017 Decision 2), not an oversight. See `quality/COMPLETION_BACKLOG.md` BG-022. |
