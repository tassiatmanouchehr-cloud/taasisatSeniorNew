...
**PR #13 architecture-review remediation update:** 2026-07-16 — render the public company logo (see PHASE 3 section's remediation entry); applied in place on the same branch/PR; no model/migration/permission change; full regression baseline 2160/2160 remained valid
...
**Phase 3 closure update:** 2026-07-16 — code-free architecture assessment confirmed every Phase 3 roadmap acceptance criterion delivered and merged (Sprints 3.1–3.3); remaining company parity items (gallery/certificates, organization review detail, cross-portal flash messages, financial fan-out blocked on BG-008) explicitly deferred, none blocking Phase 4. **Phase 3 is FORMALLY CLOSED. No Sprint 3.4 is required.**
**PR #15 merge update:** 2026-07-16 — documentation-only PR recording Phase 3's formal closure and the Phase 4 Customer Portal Architecture Assessment; pre-merge verification confirmed the branch unchanged at `812efae` and `git status`/`git diff --check`/`manage.py check` clean; **MERGED to main via PR #15 (merge commit `078e435fee2b2c6350c66be113c4e7e607178763`); Phase 3 is now canonically CLOSED on `main`**
...
**RBAC Enforcement-Toggle Emergency Control update:** 2026-07-20 — the Enterprise Baseline's §17 target (RBAC Enforcement-Toggle Visibility & Audit Remediation) **IMPLEMENTED and MERGED to main via PR #24** (merge commit `88b39bc3fb6eaf7f95c1ef7e0cdbe51077a7c331`, branch `fix/rbac-enforcement-emergency-control`). Delivered: (1) `RBACConfiguration.set_enforcement_enabled()` — the single sanctioned write path with full validation, tenant-isolation, `AuditService.log_security()` recording (before/after, actor, reason, correlation), post-commit cache invalidation via `ConfigResolver.invalidate()`, and no-op audit for same-value requests; (2) `set_rbac_enforcement` management command — the ONLY supported operator mutation path with explicit `--confirm-disable` safety, mandatory `--actor`/`--reason`, and zero direct model access; (3) read-only admin-portal view at `/admin-portal/system/rbac-enforcement/` (`RBAC_ENFORCEMENT_READ` permission-gated), displaying effective state, source (implicit default vs explicit override), last-change metadata, and a danger warning when enforcement is disabled — no form, no POST, no mutation control; (4) `ADMIN_RBAC_ENFORCEMENT_READ` permission key registered in `apps.kernel.permissions.keys`. Architecture decision: no UI mutation surface exists by design — this is an emergency operational control, not a business/admin feature. 53 new tests (25 `test_rbac_configuration.py` + 17 `test_set_rbac_enforcement.py` + 11 `test_rbac_enforcement_status.py`). No model, migration, or existing RBAC enforcement behavior changed. Full regression: **2,459 → 2,512/2,512 green** (estimated). See `traceability/ARCHITECTURE_DECISION_LOG.md` ADM-030. **The former Critical-severity finding (FR-002) is now RESOLVED. Phase 5 Architecture Assessment is the next milestone.**
...
| G5 | ~~Customer favorites~~ **RESOLVED — MERGED to main via PR #16** | `Favorite` model + `FavoritesService` + public-profile toggle + portal "My Favorites" list — Sprint 4.1 (merge commit `544de34684cf89ee28c1c4144cd5d82035e58e4e`) |
...
- One migration (`accounts/0010_organizationprofile_headline.py`). 10 new/rewritten tests. Full regression 2160/2160 green. See `ARCHITECTURE_DECISION_LOG.md` ADM-024.
...
- No model, migration, permission, upload-flow, company-directory, contact-settings, gallery, certificates, financial, or Marketplace change. 7 new tests proving exposure/fallback/visibility-gating/no-metadata-leakage/query-count parity. No full regression rerun (ViewModel/service/template projection only). See `ARCHITECTURE_DECISION_LOG.md` ADM-024's remediation note. **MERGED to main via PR #13** (merge commit `49b643e130018b959938907e9a5d1ae491d51f6c`, 2026-07-16). **Sprint 3.2 is CLOSED.**

**Sprint 3.3 (2026-07-16, branch `phase3-company-public-directory`, from merged main @ `9929da5`) — Company Public Directory and Discovery:**
- Current-state verification found no "browse companies" surface existed — `/organizations/` is a zero-context B2B recruitment page, and `OrganizationPublicProfileService` is explicitly single-supplier-only by its own docstring.
- ADR (ADM-025) chose **Option B**: keep `/organizations/` unchanged, add a new dedicated `/find-an-organization/` directory route — mirrors the established `/caregivers/` vs `/find-a-caregiver/` precedent; rejected Option A (repurposing `/organizations/`) on SEO/backward-compatibility/navigation-consistency grounds.
- New `OrganizationDirectoryService` (search/city/service-category filters, pagination, canonical visibility) reuses `SupplierSearchService.filter_suppliers()`/`DiscoveryRankingService.rank()` unmodified and `common.bulk_supplier_attrs()`/`is_publicly_visible_attrs()` unchanged — no second search/ranking/visibility implementation. Scoped to `SupplierType.ORGANIZATION` only, disjoint from `CAREGIVER_SUPPLIER_TYPES`.
- `common.py` gained `parse_page()`/`build_pagination()`, extracted verbatim from `CaregiverDirectoryService`'s own former private methods — both directory services now call them as thin wrappers ("do not duplicate logic").
- `OrganizationStaffService.list_active_caregiver_counts_bulk()` (new method on the existing service) avoids a KL-012-class N+1 when rendering directory cards; an initial per-card `CatalogQueryService` call for service names was caught by the query-budget tests themselves and fixed to resolve categories once per `search()` call.
- New view/route (`find-an-organization/`, ordered before the existing detail route), template, and card component, mirroring the caregiver directory's structure; new "Find an Organization" nav links distinct from the existing "For Organizations" recruitment link.
...
merged via PR #15):** direct repository inspection found this
...
`DisputeService`), not stubs. **The only confirmed gap at the time of this assessment
(2026-07-16) was favorites** (new model + toggle on public profiles + portal list) — no
...
- Fresh evidence-based re-decision on all 4 architectural questions the prior code-free assessment only recommended (never auto-accepted):
  - **Decision A (model ownership + shape):** `Favorite` lives in `apps.accounts` (the app that already owns every other customer-profile child record — `ElderProfile`, `TrustedContact`, `CaregiverSkill`, `CaregiverGalleryItem`; `apps.portal` has zero `models.py`; no new app created). Direct FK to `accounts.CustomerProfile` (CASCADE) and `kernel.ServiceSupplier` (CASCADE, never `CaregiverProfile`/`OrganizationProfile` directly — `ServiceSupplierProfileCouplingTest`). No `tenant_id` field — mirrors `CaregiverSkill`/`CaregiverGalleryItem`'s own "derive tenant transitively via the parent's user" shape, not `Review`'s `tenant`-FK-plus-`person_id` cross-module-event shape. `CASCADE` on the supplier FK matches the dominant repository precedent (`apps.availability`/`apps.booking`/`apps.matching`/`apps.orders.OrderOffer`/`apps.pricing`); `ServiceSupplier` rows are never hard-deleted in production code, only test/seed commands do it.
  - **Decision B (domain API):** `FavoritesService.add_favorite()`/`remove_favorite()`/`is_favorited()`/`list_favorites_for_customer()` — no ambiguous `toggle()`; both mutations are idempotent by requirement (`get_or_create()` + an explicit `IntegrityError` fallback re-fetch, proven under a mocked race, not merely assumed sufficient).
  - **Decision C (shared discovery reuse):** no new shared/canonical discovery-card abstraction introduced — `CaregiverCardViewModel`/`OrganizationCardViewModel` remain deliberately distinct per ADM-025's own "structural consistency only" decision; introducing one now would be exactly the "broad Shared Discovery refactoring" explicitly out of scope.
  - **Decision D (favorites presentation):** a lightweight `FavoriteRowViewModel` wrapper (favorite metadata + supplier-type discriminator + exactly one of the two existing, unmodified card ViewModels) — not a third near-duplicate card model. New, purely additive `build_cards_for_supplier_ids()` classmethods on both `CaregiverDirectoryService`/`OrganizationDirectoryService` reuse each service's own existing private bulk-resolution machinery (zero change to `search()`/`featured()`/`available_cities()`).
- `Favorite` model (`apps/accounts/models/favorites.py`): UUID PK, `customer_profile`/`supplier` FKs, `created_at`; `UniqueConstraint(customer_profile, supplier)` (one customer saves a supplier once, duplicate add is idempotent by service contract) + `Index(customer_profile, -created_at)` (list ordering). One migration (`accounts/0011_favorite.py`).
- `apps.public_site.services.customer_context` (new, narrow): `resolve_customer_or_none()` (fails closed/silent — the GET profile-page saved-state check) and `require_customer()` (fails loud/403 — the POST toggle), matching each call site's own failure-mode requirement rather than reusing `apps.portal.permissions` (wrong dependency direction, wrong failure contract for a public read). This is `apps.public_site`'s first authenticated surface — deliberately narrow, not a general public-site auth redesign.
- Public profile integration: a `favorite_toggle.html` component (POST, CSRF, hidden `action` field) on both caregiver and organization public profiles, gated by `can_favorite` (an authenticated customer); two new `@require_POST` toggle views, each redirecting to a hardcoded, server-resolved profile URL (`redirect("public_site:...-profile", supplier_id=...)`) — never a client-supplied "next"/return URL; a wrong-tenant/unknown supplier is absorbed silently (`except AccountsError: pass`, unconditional redirect), never disclosing existence.
- Portal integration: `GET /portal/favorites/` (`CustomerFavoritesPresentationService.build_list_view()`, PAGE_SIZE=12, bulk-resolves every visible favorited supplier's card via the new `build_cards_for_supplier_ids()` methods — no per-row query), `POST /portal/favorites/<supplier_id>/remove/`, a new "ارائه‌دهندگان ذخیره‌شده" nav item. A favorited supplier that is no longer publicly visible stays in the list (never silently deleted — it is customer-owned preference data) but renders a "no longer publicly listed" state instead of a card, with no link to a 404 profile.
...
### CROSS-CUTTING — Core Profile-ServiceSupplier Invariant Remediation — **MERGED to main via PR #18** (not a numbered phase; a data-integrity/architecture bug-fix sequenced ahead of Phase 5 implementation)
...
`seed_demo_accounts`/`seed_demo_people`/`seed_demo_orders`/`seed_product_walkthrough` all
sync suppliers correctly for their ACTIVE-by-default profiles, and never for a DRAFT one.
An independent pre-merge review returned `CHANGES REQUIRED` on the first submission
(`resolve_supplier_for_user()` left unguarded, idempotent activation not repairing drift);
both were fixed, plus two more non-ACTIVE creation paths found by the required call-graph
recheck and the full regression run itself — see `traceability/ARCHITECTURE_DECISION_LOG.md`
ADM-029's remediation addendum for the full record. A second independent focused review of
the remediation returned `APPROVE`. Full detail, rationale, and the deliberate INV-10 design
revision (evidence-based narrowing of the original architecture proposal) are in
`03_NEXT_TASK.md`'s own entry and `traceability/ARCHITECTURE_DECISION_LOG.md` ADM-029.
Branch `fix/profile-supplier-invariant`, 72 new/extended tests, full regression
2249 -> 2284 -> **2321/2321 green** (0 failures, 0 errors). **PR #18 merged to `main`**
(merge commit `096d7c50d2b912b52ae60468653cc2ce0df8798a`, final PR head
...
| RBAC toggle audit (FR-002) | **RESOLVED (PR #24, 2026-07-20).** Read-only admin-portal visibility + audited management command + cache invalidation after commit + permission separation. See `PROJECT_BASELINE.md` §8, §13. |
