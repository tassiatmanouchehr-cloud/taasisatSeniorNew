...
**Sprint 3.1 update:** 2026-07-16 — company foundation and caregiver management (see PHASE 3 section below) implemented on branch `phase3-company-portal-foundation` (from main @ `90e608d`); PR #12 created, not yet merged
**PR #12 architecture-review remediation update:** 2026-07-16 — preserve affiliation-period history (see PHASE 3 section's remediation entry); applied in place on the same branch/PR; full regression 2150/2150 green; **PR #12 still not merged — awaiting review**
**Branch verified:** phase3-company-portal-foundation (via claude/taasisat-senior-state-verify-9dzzlm)
...
- Fix: removed `unique_together`, added two conditional `UniqueConstraint`s (`uniq_active_caregiver_membership_per_user` — one ACTIVE caregiver-role membership per user, globally; `uniq_open_membership_per_org_user_role` — one open PENDING/ACTIVE membership per organization+user+role_type) that enforce only the live-state invariants while leaving terminal rows free to accumulate. `approve_affiliation_request()`/`invite_caregiver()` now always `.create()` a new row. Added `closure_reason` (new `AffiliationClosureReason` choices field) to distinguish invitation-declined/invitation-cancelled/terminated-by-company/left-by-caregiver without relying on free-text `termination_reason`. Added a matching conditional constraint on `CompanyAffiliationRequest` (`uniq_pending_affiliation_request_per_caregiver`) for the same idempotency guarantee on join requests.
- One migration (`accounts/0009_...`). 5 new/rewritten tests proving: terminate → re-invite → accept creates two separate rows; the first stays terminal and unchanged; two concurrent organizations still cannot activate the same caregiver; duplicate pending invitations/requests are rejected idempotently; historical rows grant no current company access. Full regression 2150/2150 green. See `ARCHITECTURE_DECISION_LOG.md` ADM-023's remediation note. **PR #12 not merged — awaiting review.**
