...
and mobile navigation tests. These non-snapshot tests can fail due to genuine
accessibility violations or interaction bugs — their failures are real CI
signals, not baseline-generation problems.
...
This separation ensures the generation workflow exits cleanly (no false
exit-code-1 from unrelated test failures) and produces a single-purpose
artifact containing only the verified baseline images.
