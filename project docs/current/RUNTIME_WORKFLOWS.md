...
4. If the profile is already `ACTIVE`, returns immediately with `transitioned=False` — no eligibility re-check, no duplicate `AuditLog` entry, no second `status` write. (**Independent pre-merge review of PR #18 remediation:** this path is no longer a pure no-op — it still invokes `supplier_bridge.sync_supplier_for_profile_activation()` before returning, so an already-`ACTIVE` profile with a missing or drifted `ServiceSupplier` is repaired through this same call, not only through the separate `reconcile_profile_supplier_invariant` command.)
...
6. If eligible: sets `status = ProfileStatus.ACTIVE` (a real `DRAFT -> ACTIVE` transition — registration now creates caregiver/organization profiles as `ProfileStatus.DRAFT`, not `ACTIVE`), then (**Core Profile-ServiceSupplier Invariant Remediation**) invokes `supplier_bridge.sync_supplier_for_profile_activation(profile, tenant_id=tenant_id)` inside the same transaction — guaranteeing the profile's `kernel.ServiceSupplier` both exists (created via the sanctioned `get_or_create_supplier_for_caregiver()`/`_organization()` bridge, never directly) and is itself `SupplierStatus.ACTIVE` (`SupplierRegistry.set_status()`, idempotent). A sync failure is never caught/suppressed — it propagates and rolls back the profile transition together with any partial supplier mutation, and no `AuditLog` entry is written. Only on success does the method write the `AuditLog` entry recording `before_snapshot`/`after_snapshot` status — the permanent record of *when* and *by whom* the transition happened, not the thing that determines current state. Returns a structured `ProfileActivationResult(profile, previous_status, status, transitioned=True)`.
7. Concurrent activation attempts on the same profile serialize via the row lock; exactly one real transition, one `ServiceSupplier` row (enforced by a database `UniqueConstraint(linked_entity_id, linked_entity_type)`, not `get_or_create()` alone), and one `AuditLog` entry result.
