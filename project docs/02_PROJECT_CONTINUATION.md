...
| Default branch | main |
| main HEAD SHA | ffb82a4767ba115dc158cb845b92211ccbc30d00 (merge of PR #12) |
| Feature branch HEAD | `phase3-company-professional-profile` (from main @ ffb82a4) — Sprint 3.2 implemented, PR created, not yet merged |
...
| Sprint 3.2 (PR #13, current state) | Company Professional Profile and Public Presence IMPLEMENTED on `phase3-company-professional-profile` (branched from merged main @ ffb82a4): current-state inspection found most target capabilities already built (`OrganizationProfile` public/contact fields, `OrganizationProfileUpdateService`, logo/cover upload, the public organization-profile page) — closed the genuinely missing/broken pieces instead of rebuilding: added `OrganizationProfile.headline` (professional headline/short intro, one migration); fixed `OrganizationPublicProfileService.get_profile()` to use the canonical `common.is_publicly_visible_attrs()` (it previously only checked `profile_status`, so an unverified organization or one with a deactivated admin account was incorrectly publicly visible); fixed the organization public-profile page's SEO canonical URL (KL-021/BG-027, previously deferred); permission-gated the organization logo/cover upload/remove methods (`ORGANIZATION_PROFILE_UPDATE`, reused — they previously had no permission check at all, only `resolve_organization()`'s ownership boundary); made `ProfileMediaService._replace()` transaction-safe (old file deleted via `transaction.on_commit()`, mirroring Sprint 2.2's gallery remediation). One migration (`accounts/0010_organizationprofile_headline.py`). 10 new/rewritten tests, full regression 2160/2160 green at the point of the initial PR. **PR #13 architecture-review remediation:** the public organization profile now renders the organization's own already-uploaded logo (`logo_url` exposed via the existing `logo` field's own storage URL, initials fallback only when no logo/no usable URL) — Sprint 3.2's original "initials-only, matches caregiver precedent" decision was superseded, not a redesign. Canonical visibility policy still gates the logo along with the rest of the profile. No model/migration/permission change; 7 more tests (17 total across Sprint 3.2 + its remediation). PR #13 created, **not merged.** See `traceability/ARCHITECTURE_DECISION_LOG.md` ADM-024 and its remediation note. |
...
| Active work branch | `phase3-company-professional-profile` (from main @ ffb82a4) — Sprint 3.2, PR awaiting review |
