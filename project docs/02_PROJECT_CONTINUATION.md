...
| Feature branch HEAD | `phase3-company-portal-foundation` @ `5478a6f` (from main @ 90e608d) — Sprint 3.1 implemented on PR #12; PR #12 architecture-review remediation applied; PR #12 not yet merged |
...
| Sprint 3.1 (PR #12, current state — supersedes any earlier count) | Company Foundation and Caregiver Management, plus the PR #12 architecture-review remediation, both IMPLEMENTED on `phase3-company-portal-foundation` (branched from merged main @ 90e608d): extends the pre-existing `OrganizationMembership`/`CompanyAffiliationRequest` models (no new models) with a full affiliation lifecycle — join-by-code, company invitation, approval/rejection, mutual termination, per-cycle history (each affiliation period is a separate, immutable `OrganizationMembership` row; terminal rows are never reactivated; two conditional `UniqueConstraint`s enforce live-state invariants; `closure_reason` distinguishes terminal outcomes; `CompanyAffiliationRequest` has a matching conditional pending-request constraint) — across `apps.accounts.services.affiliations`, new organization_portal and provider_portal UI, and 4 new permission keys. Two migrations (`accounts/0008_company_affiliation_termination.py`, `accounts/0009_alter_organizationmembership_unique_together_and_more.py`). 56 new/rewritten tests total. **Full regression 2150/2150 green (current, final count).** PR #12 created, architecture-review remediation applied, **PR #12 not merged.** See `traceability/ARCHITECTURE_DECISION_LOG.md` ADM-023 and its remediation note. |
