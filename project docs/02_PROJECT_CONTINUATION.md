...
| Feature branch HEAD | `phase3-company-portal-foundation` (from main @ 90e608d) — Sprint 3.1 implemented on PR #12; PR #12 architecture-review remediation applied; PR #12 not yet merged |
...
| Current phase | **Phase 2 — CLOSED and MERGED to main** (PR #11, merge commit `90e608d`). **Phase 3 — Company Portal is ACTIVE**; **Sprint 3.1 — Company Foundation and Caregiver Management** is implemented on **PR #12** (architecture-review remediation applied, PR #12 not yet merged — see `IMPLEMENTATION_ROADMAP.md`) |
...
| Sprint 3.1 | Company Foundation and Caregiver Management IMPLEMENTED on `phase3-company-portal-foundation` (branched from merged main @ 90e608d): extends the pre-existing `OrganizationMembership`/`CompanyAffiliationRequest` models (no new models) with a full affiliation lifecycle — join-by-code, company invitation, approval/rejection, mutual termination, history — across `apps.accounts.services.affiliations`, new organization_portal and provider_portal UI, and 4 new permission keys. One migration (3 new nullable termination fields). 51 new tests, full regression 2145/2145 green. PR #12 created. See `traceability/ARCHITECTURE_DECISION_LOG.md` ADM-023. |
| PR #12 remediation | Architecture review of PR #12 identified two merge blockers, both resolved in place on the same branch/PR (2026-07-16): **Blocker 1** — `OrganizationMembership` reactivation replaced with always-`.create()` per affiliation cycle, two conditional `UniqueConstraint`s replace `unique_together`, new `closure_reason` field, 5 new/rewritten tests, one migration (`accounts/0009_...`). **Blocker 2** — active documentation contradictions cleaned across the 9 governance-listed files. Full regression 2150/2150 green. See `traceability/ARCHITECTURE_DECISION_LOG.md` ADM-023's remediation note. **PR #12 not merged.** |
...
| Active work branch | `phase3-company-portal-foundation` (from main @ 90e608d) — PR #12, awaiting review |
