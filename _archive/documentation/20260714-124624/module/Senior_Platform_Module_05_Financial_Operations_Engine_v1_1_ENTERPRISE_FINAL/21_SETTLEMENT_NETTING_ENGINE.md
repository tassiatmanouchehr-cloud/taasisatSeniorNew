...
# 21 — Settlement, Batch & Netting Engine

## Definition

Settlement, Batch & Netting Engine is a Module 05 sub-engine responsible for net position calculation, settlement batches, independent settlement items, failure and retry, cash collection offsets.

## Responsibilities

- Net position calculation.
- Settlement batches.
- Independent settlement items.
- Failure and retry.
...
- `settlement,_batch_and_netting_engine_started`
- `settlement,_batch_and_netting_engine_policy_resolved`
- `settlement,_batch_and_netting_engine_completed`
- `settlement,_batch_and_netting_engine_failed`
...
## v1.1 Multi-Party Netting Correction

Netting must support more than bilateral platform-provider balances.

Required parties include:
...
Settlement amount must always be based on eligible net balance, not completed order total.
